...
func newCertTemplate() (sm2.Certificate, error) {
	...
	return sm2.Certificate{
		...
		KeyUsage:     sm2.KeyUsage(sm2.KeyUsageKeyEncipherment )|sm2.KeyUsage(sm2.KeyUsageDigitalSignature) ,
		SerialNumber: sn,
	}, nil
}
...
func newCertKeyPair(isCA bool, isServer bool, host string, certSigner crypto.Signer, parent *sm2.Certificate) (*certKeyPair, error) {
	...
	if isCA {
		...
		template.IsCA = true
		template.KeyUsage |= sm2.KeyUsage(sm2.KeyUsageCertSign) |sm2.KeyUsage(sm2.KeyUsageCRLSign)
